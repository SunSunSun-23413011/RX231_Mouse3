...
void mode1(int x){
  ...
  int mot_cnt;
  Start_Sound(2);
  PIN_WRITE(MOTOR_EN) = 1;                 // モータEnable ON
  //左モータチェック
  //手動で10ステップ左モータ正転駆動
  PIN_WRITE(L_MOT_MODE) = LeftGo;   // 左モータ正転方向
  for(mot_cnt=0; mot_cnt<10; mot_cnt++){
    R_Config_MTU3_Start();        // パルスON
    pause(100);
    R_Config_MTU3_Stop();        // パルスOFF
    pause(100);
  }
  pause(1000);
  //手動で10ステップ左モータ反転駆動
  PIN_WRITE(L_MOT_MODE) = LeftBack;   // 左モータ反転方向
  for(mot_cnt=0; mot_cnt<10; mot_cnt++){
    R_Config_MTU3_Start();        // パルスON
    pause(100);
    R_Config_MTU3_Stop();        // パルスOFF
  ...
  //右モータチェック
  //手動で10ステップ右モータ正転駆動
  PIN_WRITE(R_MOT_MODE) = RightGo;   // 右モータ正転方向
  for(mot_cnt=0; mot_cnt<10; mot_cnt++){
    R_Config_MTU4_Start();        // パルスON
    pause(100);
    R_Config_MTU4_Stop();        // パルスOFF
    pause(100);
  }
  pause(1000);
  //手動で10ステップ右モータ反転駆動
  PIN_WRITE(R_MOT_MODE) = RightBack;   // 右モータ反転方向
  for(mot_cnt=0; mot_cnt<10; mot_cnt++){
    R_Config_MTU4_Start();        // パルスON
    pause(100);
    R_Config_MTU4_Stop();        // パルスOFF
  ...
  PIN_WRITE(MOTOR_EN) = 0;                 // モータEnable OFF
  Stop_Sound();
  MODE = 0; // モード0に戻す
}
...
void mode2(int x){
  ...
  PIN_WRITE(R_MOT_MODE) = RightGo;               // 右モータ正転方向
  //速度遅く(パルス周期長く)正転駆動
  for(mot_cnt=0; mot_cnt<400; mot_cnt++){
    R_Config_MTU3_Start();        // パルスON
    R_Config_MTU4_Start();        // パルスON
    pause(10);
    R_Config_MTU3_Stop();        // パルスOFF
