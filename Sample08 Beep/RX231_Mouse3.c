...
vshort   Batt;             // 電池の電圧
static volatile uint16_t g_beep_remaining_ms = 0;
...
void mode1( int x );
void mode2( int x );
void Beep_Start(uint16_t freq_hz, uint16_t duration_ms);
void Beep_Stop(void);
static void Beep_Task_1ms(void);
static uint16_t Beep_Calc_TGRB(uint16_t freq_hz);
//---------------------------------------------------------------
//  メインプログラム
//---------------------------------------------------------------
void main(void){
...
static uint16_t Beep_Calc_TGRB(uint16_t freq_hz)
{
  ...
  tgrb = (BSP_PCLKA_HZ / (MTU0_PCLK_COUNTER_DIVISION * 2U * (uint32_t)freq_hz));
  if (tgrb == 0U) {
    tgrb = 1U;
  }
  if (tgrb > 0x10000U) {
