...
void mouse_search( int goal_x, int goal_y, int spd ){
  ...
  while( 1 ){
    ...
    // 残り半区間を直進
    speed = speed_now;
    while( STEP < GO_STEP - speed_now * 2 );  // 減速ステップ数を残して定速移動
    // 減速
    speed = 1;
    while( STEP < GO_STEP );                  // 残りのステップ数で減速

    // 次のループに移る前に旋回
    if( wall_data == 0x09 ){      // 左と前に壁があったら
      com_turn( 0 );  // 右90度旋回
    }
  }
}
