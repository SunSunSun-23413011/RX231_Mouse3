...
// 探索関連
#define   GOAL_X      8    // ゴール x座標
#define   GOAL_Y      8    // ゴール y座標
...
void mode5( int x );
void mode6( int x );
void mouse_search( int goal_x, int goal_y, int speed );
...
void make_potential( int goal_x, int goal_y );
    ...
    switch( SENSOR_PT ){          // タスクポインタに従って処理を行う
      ...
      case 3: if ( (R_SW == LED_OFF) && (L_SW == LED_OFF) && (F_SW == LED_OFF) ) break;
              ...
              R_Config_S12AD0_Start(); // ADC開始
               ...
               if(R_SW == LED_ON){
                R_Config_S12AD0_Get_ValueResult(ADCHANNEL0, (uint16_t *)&g_s12ad0_ch000_value);
                R_PRE = ((g_s12ad0_ch000_value) - R_PRE) / 2;
               ...
               if(L_SW == LED_ON){
                R_Config_S12AD0_Get_ValueResult(ADCHANNEL1, (uint16_t *)&g_s12ad0_ch001_value);
                L_PRE = ((g_s12ad0_ch001_value) - L_PRE) / 4;
               ...
               if(F_SW == LED_ON){
                R_Config_S12AD0_Get_ValueResult(ADCHANNEL2, (uint16_t *)&g_s12ad0_ch002_value);
                F_PRE = ((g_s12ad0_ch002_value) - F_PRE) / 2;
                if( F_PRE < 0 )         F_SEN = 0;
...
void mode5( int x ){
  ...
  Start_Sound(2);
  mouse_search( GOAL_X, GOAL_Y, 200 );
  Start_Sound(96);
}
...
void mode6(int x){
  ...
  while(1){
    ...
    if( PIN_READ(SW_RETURN) == 0 ) { return; }
  }
}
...
//  探索関数
//-------------------------------------------------------------------------
void mouse_search( int goal_x, int goal_y, int spd ){
...
//  等高線（ポテンシャル場）作成
//-------------------------------------------------------------------------
void make_potential( int gx, int gy )
{
  uchar check_num, flg;
  uchar x,y;
